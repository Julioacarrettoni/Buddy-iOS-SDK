...
- (void)testIdentityGetAll
{
    ...
    if (self.user == nil)
    {
        STFail(@"testIdentityGetAll failed to login");
        return;
    }

    bwaiting = true;
    [self atestIdentityGetAll];
    [self waitloop];


    bwaiting = true;
    [self atestStateIdentityRemove:self.user];
    [self waitloop];
}

- (void)atestIdentityGetAll
{
    [self IdentityGetAllTest:self.user];
}

- (void)IdentityGetAllTest:(BuddyAuthenticatedUser *)buddyuser
{
    NSObject *state = [[NSObject alloc] init];

    [buddyuser.identityValues getAll:state
                            callback:[^(BuddyArrayResponse *response)
                            {
                                if (response.isCompleted && response.result != nil)
                                {
                                    ...
                                    {
                                        STFail(@"atestIdentityGetAll failed result is not an NSArray");
                                    }
                                    else
                                    {
                                        NSLog(@"IdentityGetAllTest OK count: %d", [response.result count]);
                                    }
                                }
                                else
                                {
                                    STFail(@"IdentityGetAllTest failed !response.isCompleted || response.result == nil");
                                }
                                bwaiting = false;
                            } copy]];
}

- (void)testIdentityCheckForValues
{
    bwaiting = true;
    [self alogin];
    [self waitloop];
    ...
    if (self.user == nil)
    {
        STFail(@"testIdentityCheckForValues failed to login");
        return;
    }

    bwaiting = true;
    [self atestIdentityAdd];
    [self waitloop];

    bwaiting = true;
    [self atestIdentityCheckForValues];
    [self waitloop];

    [self atestIdentityRemove];
    [self waitloop];
}

- (void)atestIdentityCheckForValues
{
    [self IdentityCheckForValuesTest:self.user];
}

- (void)IdentityCheckForValuesTest:(BuddyAuthenticatedUser *)buddyuser
{
    NSObject *state = [[NSObject alloc] init];

    [buddyuser.identityValues checkForValues:@"John Smith" state:state
                                    callback:[^(BuddyArrayResponse *response)
                                    {
                                        if (response.isCompleted && response.result != nil)
                                        {
                                            if (![response.result isKindOfClass:[NSArray class]])
                                            {
                                                STFail(@"atestIdentityCheckForValues failed result is not an NSArray");
                                            }
                                            else
                                            {
                                                NSLog(@"IdentityCheckForValuesTest OK");
                                                NSArray *identityArray = response.result;
                                                if ([identityArray count] > 0)
                                                {
                                                    BuddyIdentityItem *identityItem = [identityArray objectAtIndex:0];
                                                    NSLog(@"Identity value: %@", identityItem.value);
                                                }
                                            }
                                        }
                                        else
                                        {
                                            STFail(@"IdentityCheckForValuesTest failed !response.isCompleted || response.result == nil");
                                        }
                                        bwaiting = false;
                                    } copy]];
}
...
- (void)testIdentityAdd
{
    ...
    if (self.user == nil)
    {
        STFail(@"testIdentityAdd failed to login");
        return;
    }
...
- (void)atestIdentityAdd
{
    [self IdentityAddTest:self.user];
}

- (void)IdentityAddTest:(BuddyAuthenticatedUser *)buddyUser
{
    NSObject *state = [[NSObject alloc] init];

    [buddyUser.identityValues add:@"John Smith" state:state
                         callback:[^(BuddyBoolResponse *response)
                         {
                             if (response.isCompleted && response.result)
                             {
                                 NSLog(@"IdentityAddTest OK");
                             }
                             else
                             {
                                 if (response.isCompleted == FALSE)
                                 {
                                     STFail(@"IdentityAddTest failed !response.isCompleted && !response.result");
                                 }
                             }
                             bwaiting = false;
                         } copy]];
...
- (void)atestIdentityRemove
{
    bwaiting = true;

    [self atestIdentityRemove:self.user];
...
- (void)IdentityRemoveTest:(BuddyAuthenticatedUser *)buddyuser
{
    ...
    [buddyuser.identityValues remove:@"John Smith" state:state
                            callback:[^(BuddyBoolResponse *response)
                            {
...
- (void)atestStateIdentityRemove:(BuddyAuthenticatedUser *)buddyuser
{
    ...
    [self StateIdentityRemoveTest:buddyuser state:state];
}

- (void)StateIdentityRemoveTest:(BuddyAuthenticatedUser *)buddyuser state:(NSObject *)state
{
    [buddyuser.identityValues remove:@"John Smith" state:state
                            callback:[^(BuddyBoolResponse *response)
                            {
